...
----
+
You can optionally provide a `profile_name`, `role-session-name`, and `session-duration`. If you provide a `profile_name`, the temporary keys and token will be configured to that specific profile. You must use that same `profile_name` with the rest of your aws commands. If a `profile_name` is not provided, the keys and token will be configured to the default profile.
+
